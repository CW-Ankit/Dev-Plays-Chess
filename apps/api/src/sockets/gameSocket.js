import { getSessionFromSocket } from "../services/sessionService.js";
import {
    enqueuePlayer,
    removeWaitingPlayer,
    findMatchFor,
...
const emitWaiting = (socket, message) => {
    socket.emit(SocketEvents.QUEUE_STATUS, { waiting: true, message });
};

const emitBoardState = (io, game) => {
    io.to(game.id).emit(SocketEvents.BOARD_STATE, {
        fen: game.chess.fen(),
...
const emitMatchFound = (game) => {
    ...
    white.socket.emit(SocketEvents.MATCH_FOUND, {
        roomId: game.id,
        role: "w",
        opponent: black.name
    });

    black.socket.emit(SocketEvents.MATCH_FOUND, {
        roomId: game.id,
        role: "b",
        opponent: white.name
    });
...
const queuePlayer = (socket) => {
    ...
    socket.nsp.to(game.id).emit(SocketEvents.QUEUE_STATUS, { waiting: false, message: "Match started" });
...
export const initializeGameSocket = (io) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on(SocketEvents.JOIN_QUEUE, (preferredColor) => {
            ...
            if (currentGame) {
                socket.nsp.to(currentGame.id).emit(SocketEvents.OPPONENT_LEFT, { message: "Opponent left. Finding new match..." });
        ...
        socket.on(SocketEvents.MOVE, async (move) => {
            ...
            try {
                ...
                io.to(game.id).emit(SocketEvents.MOVE, result);
                emitBoardState(io, game);

                if (game.chess.isGameOver()) {
                    await finalizeGame(game, "completed");
                    io.to(game.id).emit(SocketEvents.GAME_OVER, {
                        checkmate: game.chess.isCheckmate(),
                        draw: game.chess.isDraw(),
        ...
        socket.on("game:rematch", () => {
            ...
            if (game.rematchVotes.size === 2) {
                ...
                game.players.w.socket.emit(SocketEvents.MATCH_FOUND, { roomId: game.id, role: "w", opponent: game.players.b.name });
                game.players.b.socket.emit(SocketEvents.MATCH_FOUND, { roomId: game.id, role: "b", opponent: game.players.w.name });
                emitBoardState(io, game);
                io.to(game.id).emit(SocketEvents.QUEUE_STATUS, { waiting: false, message: "Rematch started" });
            }
        });
        ...
        socket.on("game:newOpponent", () => {
            ...
            if (game) {
                ...
                otherSocket.emit(SocketEvents.OPPONENT_LEFT, { message: "Opponent is finding a new game." });
                queuePlayer(otherSocket);
            }
        ...
        socket.on("disconnect", () => {
            ...
            if (game) {
                const otherSocket = game.players.w.socket.id === socket.id ? game.players.b.socket : game.players.w.socket;
                otherSocket.leave(game.id);
                otherSocket.emit(SocketEvents.OPPONENT_LEFT, { message: "Opponent disconnected. Re-queueing..." });
                queuePlayer(otherSocket);
            }
        });
    });
